# Public Contributing Standard

This document defines the standard template for the CONTRIBUTING file in public Core Red repositories. Projects **must** include a `CONTRIBUTING.md` at the repository root that conforms to this standard.

This standard applies only to publicly accessible repositories. For private or internal repositories, see [PRIVATE_CONTRIBUTING_STANDARD.md](PRIVATE_CONTRIBUTING_STANDARD.md).

---

## Template Structure

The CONTRIBUTING file for public repositories **must** contain the following sections, in this order:
...
Reference the project's CODE_OF_CONDUCT file. State that the project adheres to a code of conduct and that participation requires upholding it. Link directly to `CODE_OF_CONDUCT.md`.
...
4. Create a branch for the changes.
5. Make the changes.
6. Test the changes.
...
If the project has additional build targets (e.g., desktop application, standalone UI, WASM), each **must** be documented in a separate subsection with its own fenced code block.
...
Define the full pull request procedure as an ordered list. The following steps **must** be included:

1. **Create an issue first** for significant changes.
2. **Fork and branch**: Create a feature branch from the default branch.
3. **Follow code style**: Ensure the code follows the project's style guidelines.
4. **Write tests**: Add tests for new functionality.
5. **Update documentation**: Update relevant documentation.
6. **Run all tests**: Ensure all tests pass.
7. **Commit properly**: Follow the commit message guidelines.
8. **Submit pull request**: Create a pull request with a clear description.

#### Pull Request Requirements

Provide a checklist that contributors **must** verify before submitting a pull request:
...
A CONTRIBUTING file in a public repository is non-compliant if any of the following conditions are true:
...
- The pull request process is absent or does not include a requirements checklist.
